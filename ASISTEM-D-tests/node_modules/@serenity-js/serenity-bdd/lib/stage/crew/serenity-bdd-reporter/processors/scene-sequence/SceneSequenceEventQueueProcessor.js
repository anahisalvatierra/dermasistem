...
class SceneSequenceEventQueueProcessor extends EventQueueProcessor_1.EventQueueProcessor {
    ...
    process(queue) {
        return queue.reduce((context, event) => (0, tiny_types_1.match)(event)
            ...
            .when(events_1.RetryableSceneDetected, this.onRetryableSceneDetected(context))
            .when(events_1.SceneStarts, this.onSceneStarts(context))
            .when(events_1.ActorEntersStage, this.onActorEntersStage(context))
            .when(events_1.ActorSpotlighted, this.onActorSpotlighted(context))
            ...
            .else(() => context), new SceneSequenceReportContext_1.SceneSequenceReportContext(this.requirementsHierarchy) // eslint-disable-line @stylistic/indent
        ).build();
    }
    ...
    onRetryableSceneDetected(context) {
        return (event) => context.usingRetryModeOutcomeReporting();
    }
    ...
    onSceneParametersDetected(context) {
        return (event) => context
            .with((0, transformations_2.scenarioParametersOf)(event.details, event.parameters));
    }
    ...
    onSceneFinished(context) {
        return (event) => context
            ...
            .with((0, transformations_2.scenarioParameterResult)(event.details, event.outcome))
