...
module.exports = {
    ...
    handler: async (argv) => {
        const printer = new Printer_1.Printer(process.stdout, process.stderr), artifactGAV = model_1.GAV.fromString(argv.artifact), pathToArtifact = new io_1.Path(argv.cacheDir).join(artifactGAV.toPath()), moduleRoot = node_path_1.default.resolve(__dirname, '../../../');
        (0, core_1.configure)({
        ...
        try {
            await actor.attemptsTo(screenplay_1.InvokeSerenityBDD.at(pathToArtifact)
                ...
                'LOG_LEVEL': argv.log,
                'logback.configurationFile': node_path_1.default.resolve(moduleRoot, './resources/logback.config.xml'),
