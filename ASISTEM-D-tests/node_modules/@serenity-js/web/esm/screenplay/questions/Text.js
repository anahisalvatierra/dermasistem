...
export class Text {
    ...
    static of(pageElement) {
        return Question.about(the `the text of ${pageElement}`, async (actor) => {
            const element = await actor.answer(pageElement);
            return element.text();
        }, (parent) => Text.of(PageElement.of(pageElement, parent)));
    }
    static ofAll(pageElements) {
        if (Question.isAMetaQuestion(pageElements)) {
            return Question.about(the `the text of ${pageElements}`, textOfMultiple(pageElements), (parent) => Text.ofAll(pageElements.of(parent)));
        }
