...
export class Select {
    ...
    static value(value) {
        return {
            from: (pageElement) => Interaction.where(the `#actor selects value ${value} from ${pageElement}`, async (actor) => {
    ...
    static values(...values) {
        return {
            from: (pageElement) => Interaction.where(`#actor selects values ${commaSeparated(values.flat(), item => stringified(item, { inline: true }))} from ${stringified(pageElement, { inline: true })}`, async (actor) => {
    ...
    static option(value) {
        return {
            from: (pageElement) => Interaction.where(the `#actor selects ${value} from ${pageElement}`, async (actor) => {
                const element = await actor.answer(pageElement);
    ...
    static options(...values) {
        return {
            from: (pageElement) => Interaction.where(`#actor selects ${commaSeparated(values.flat(), item => stringified(item, { inline: true }))} from ${stringified(pageElement, { inline: true })}`, async (actor) => {
                const answers = await asyncMap(values, value => actor.answer(value));
