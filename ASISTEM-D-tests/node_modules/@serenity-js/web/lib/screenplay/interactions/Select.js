...
class Select {
    ...
    static value(value) {
        return {
            from: (pageElement) => core_1.Interaction.where((0, core_1.the) `#actor selects value ${value} from ${pageElement}`, async (actor) => {
                const element = await actor.answer(pageElement);
                const desiredValue = await actor.answer(value);
                await element.selectOptions(index_js_1.SelectOption.withValue(desiredValue));
            }),
        };
    }
    /**
    ...
    static values(...values) {
        return {
            from: (pageElement) => core_1.Interaction.where(`#actor selects values ${(0, io_1.commaSeparated)(values.flat(), item => (0, io_1.stringified)(item, { inline: true }))} from ${(0, io_1.stringified)(pageElement, { inline: true })}`, async (actor) => {
                const answers = await (0, io_1.asyncMap)(values, value => actor.answer(value));
                const desiredValues = answers.flat();
                const element = await actor.answer(pageElement);
                await element.selectOptions(...desiredValues.map(value => index_js_1.SelectOption.withValue(value)));
            }),
        };
    }
    /**
    ...
    static option(value) {
        return {
            from: (pageElement) => core_1.Interaction.where((0, core_1.the) `#actor selects ${value} from ${pageElement}`, async (actor) => {
                const element = await actor.answer(pageElement);
                const desiredLabel = await actor.answer(value);
                await element.selectOptions(index_js_1.SelectOption.withLabel(desiredLabel));
            }),
        };
    }
    /**
    ...
    static options(...values) {
        return {
            from: (pageElement) => core_1.Interaction.where(`#actor selects ${(0, io_1.commaSeparated)(values.flat(), item => (0, io_1.stringified)(item, { inline: true }))} from ${(0, io_1.stringified)(pageElement, { inline: true })}`, async (actor) => {
                const answers = await (0, io_1.asyncMap)(values, value => actor.answer(value));
                const desiredLabels = answers.flat();
                const element = await actor.answer(pageElement);
                await element.selectOptions(...desiredLabels.map(label => index_js_1.SelectOption.withLabel(label)));
            }),
        };
    }
}
