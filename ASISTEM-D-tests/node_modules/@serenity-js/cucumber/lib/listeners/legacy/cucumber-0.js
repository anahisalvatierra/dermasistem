...
function default_1({ serenity, notifier, resultMapper, loader, cache }) {
    return function () {
        ...
        this.registerHandler('BeforeScenario', function (scenario) {
            ...
            if (isOutline) {
                notifier.outlineDetected(sceneId, map.get(index_js_2.Scenario).onLine(line), map.get(index_js_2.ScenarioOutline).onLine(lines[1]), map.getFirst(index_js_2.Feature));
            }
            notifier.scenarioStarts(sceneId, map.get(index_js_2.Scenario).onLine(line), map.getFirst(index_js_2.Feature));
        });
        ...
        this.registerHandler('ScenarioResult', function (result, callback) {
            ...
            serenity.waitForNextCue()
                .then(() => {
                notifier.scenarioFinished(map.get(index_js_2.Scenario).onLine(line), map.getFirst(index_js_2.Feature), outcome);
                callback();
            }, error => {
                notifier.scenarioFinished(map.get(index_js_2.Scenario).onLine(line), map.getFirst(index_js_2.Feature), outcome);
                callback(error);
...
function findStepMatching(step, map) {
    ...
    const matchedStep = map.get(index_js_2.Scenario).onLine(scenarioLine).steps.find(s => s.location.line === stepLine);
    if (!matchedStep) {
...
function ambiguousStepsDetectedIn(result) {
    ...
    return ambiguousStepDefinitions
        ...
        error.message += `\n${issue}`;
        return error;
    }, new index_js_1.AmbiguousStepDefinitionError('Multiple step definitions match:'));
}
