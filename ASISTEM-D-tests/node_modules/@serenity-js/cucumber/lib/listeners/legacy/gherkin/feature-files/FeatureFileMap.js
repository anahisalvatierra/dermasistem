"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.FeatureFileMap = void 0;
const index_js_1 = require("./errors/index.js");
/**
 * @private
 */
class FeatureFileMap {
    map;
    constructor(map = {}) {
        this.map = map;
    }
    set(item) {
        return ({
            onLine: (line) => {
                this.map[line] = item;
                return this;
            },
        });
    }
    get(type) {
        return ({
            onLine: (line) => {
                const found = this.map[line];
                if (!found) {
                    throw new index_js_1.ItemNotFoundError(`Nothing was found on line ${line}`);
                }
                if (!(found instanceof type)) {
                    throw new index_js_1.ItemNotFoundError(`Item on line ${line} is a ${found.constructor.name}, not a ${type.name}`);
                }
                return found;
            },
        });
    }
    getFirst(type) {
        const items = Object.keys(this.map).map(line => this.map[line]), found = items.find(value => value instanceof type);
        if (!found) {
            const existingTypes = items.map(item => item.constructor.name).join(', ') || 'no items.';
            throw new index_js_1.ItemNotFoundError(`Didn't find any ${type.name} amongst ${existingTypes}`);
        }
        return found;
    }
    size() {
        return Object.keys(this.map).length;
    }
}
exports.FeatureFileMap = FeatureFileMap;
//# sourceMappingURL=FeatureFileMap.js.map