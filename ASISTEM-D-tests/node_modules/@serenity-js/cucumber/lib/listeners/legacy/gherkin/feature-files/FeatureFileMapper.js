...
const FeatureFileMap_js_1 = require("./FeatureFileMap.js");
/**
 * @private
 */
class FeatureFileMapper {
    map(document, path) {
        const map = new FeatureFileMap_js_1.FeatureFileMap();
        ...
        document.feature.children.forEach(scenarioDefinition => {
            switch (scenarioDefinition.type) {
                case 'Background':
                    background = new index_js_1.Background(new io_1.FileSystemLocation(path, scenarioDefinition.location.line, scenarioDefinition.location.column), new model_1.Name(scenarioDefinition.name), scenarioDefinition.description && new model_1.Description(scenarioDefinition.description), scenarioDefinition.steps.map(step => this.asStep(path, step)));
                    map.set(background).onLine(scenarioDefinition.location.line);
                    break;
                case 'Scenario':
                    map.set(new index_js_1.Scenario(new io_1.FileSystemLocation(path, scenarioDefinition.location.line, scenarioDefinition.location.column), new model_1.Name(scenarioDefinition.name), scenarioDefinition.description && new model_1.Description(scenarioDefinition.description), (background ? background.steps : []).concat(scenarioDefinition.steps.map(step => this.asStep(path, step))), this.tagsFrom(document.feature.tags, scenarioDefinition.tags))).onLine(scenarioDefinition.location.line);
                    break;
                    ...
                    outline.examples.filter(e => e.tableHeader !== undefined).forEach(examples => {
                        ...
                        examples.tableBody.forEach(values => {
                            ...
                            outline.steps.forEach(scenarioOutlineStep => {
                                ...
                                steps.push(new index_js_1.Step(new io_1.FileSystemLocation(path, scenarioOutlineStep.location.line, scenarioOutlineStep.location.column), new model_1.Name([
                            ...
                            map.set(new index_js_1.Scenario(new io_1.FileSystemLocation(path, values.location.line, values.location.column), new model_1.Name(outline.name), outline.description && new model_1.Description(outline.description), steps, this.tagsFrom(document.feature.tags, outline.tags, examples.tags), new io_1.FileSystemLocation(path, outline.location.line, outline.location.column))).onLine(values.location.line);
                        });
                    });
                    map.set(new index_js_1.ScenarioOutline(new io_1.FileSystemLocation(path, outline.location.line, outline.location.column), new model_1.Name(outline.name), outline.description && new model_1.Description(outline.description), (background ? background.steps : []).concat(outline.steps.map(step => this.asStep(path, step, [], []))), parameters)).onLine(scenarioDefinition.location.line);
                    break;
                }
            }
        });
        map.set(new index_js_1.Feature(new io_1.FileSystemLocation(path, document.feature.location.line, document.feature.location.column), new model_1.Name(document.feature.name), document.feature.description && new model_1.Description(document.feature.description), background)).onLine(document.feature.location.line);
        return map;
    }
    asStep(path, step, variableCells = [], valueCells = []) {
        return new index_js_1.Step(new io_1.FileSystemLocation(path, step.location.line, step.location.column), new model_1.Name([
            step.keyword,
            step.text,
