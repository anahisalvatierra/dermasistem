...
export class Serenity {
    static defaultCueTimeout = Duration.ofSeconds(5);
    static defaultInteractionTimeout = Duration.ofSeconds(5);
    static defaultActors = new Extras();
    stage;
    fileSystem;
     ...
     */
    configure(config) {
        const looksLikeBuilder = has({ build: 'function' });
        const looksLikeStageCrewMember = has({ assignedTo: 'function', notifyOf: 'function' });
        ...
        if (Array.isArray(config.crew)) {
            this.stage.assign(...config.crew.map((stageCrewMemberDescription, i) => {
                ...
                throw new ConfigurationError(d `Entries under \`crew\` should implement either StageCrewMember or StageCrewMemberBuilder interfaces, \`${stageCrewMemberDescription}\` found at index ${i}`);
            }));
        }
    }
    /**
