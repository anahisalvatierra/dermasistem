...
export class ClassDescriptionParser {
    ...
    parse(classDescription) {
        ...
        if (rest.length > 0) {
            throw new ConfigurationError(`${classDescription} is not a valid class description. Valid class description must be:\n` +
        ...
        if (desiredClassName === '') {
            throw new ConfigurationError(`Invalid class name in "${moduleIdAndClassDescription}". If you want to import the default export from a given module, please use the module ID.\n` +
        ...
        if (!ClassDescriptionParser.validClassNamePattern.test(className)) {
            throw new ConfigurationError(`"${className}" doesn't seem like a valid JavaScript class name in "${moduleIdAndClassDescription}"`);
        }
        if (!moduleId.startsWith('./')) {
            const result = validate(moduleId);
            if (!result.validForNewPackages) {
                throw new ConfigurationError(`"${moduleId}" doesn't seem like a valid module id:\n${result.errors.map(error => `- ${error}\n`)}`);
            }
        }
        return {
            moduleId,
