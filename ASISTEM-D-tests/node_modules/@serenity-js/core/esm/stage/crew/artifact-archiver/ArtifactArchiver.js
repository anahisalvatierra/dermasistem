...
import { ActivityRelatedArtifactArchived, ActivityRelatedArtifactGenerated, ArtifactArchived, ArtifactGenerated, AsyncOperationAttempted, AsyncOperationCompleted, AsyncOperationFailed } from '../../../events/index.js';
...
export class ArtifactArchiver {
    ...
    archive(relativePath, contents, encoding, announce) {
        ...
        this.stage.announce(new AsyncOperationAttempted(new Name(this.constructor.name), new Description(`Saving '${relativePath.value}'...`), id, this.stage.currentTime()));
    ...
    archivisationAnnouncement(event, relativePathToArtifact) {
        return (absolutePath) => {
            if (event instanceof ActivityRelatedArtifactGenerated) {
                this.stage.announce(new ActivityRelatedArtifactArchived(event.sceneId, event.activityId, event.name, event.artifact.constructor, relativePathToArtifact, event.timestamp, this.stage.currentTime()));
            }
            else if (event instanceof ArtifactGenerated) {
                this.stage.announce(new ArtifactArchived(event.sceneId, event.name, event.artifact.constructor, relativePathToArtifact, event.timestamp, this.stage.currentTime()));
            }
        };
    }
}
