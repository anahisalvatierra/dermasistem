...
class Actor {
    ...
    constructor(name, stage, abilities = []) {
        ...
        this.stage = stage;
        [
            new index_js_5.PerformActivities(this, stage),
            new index_js_5.AnswerQuestions(this),
            ...abilities
    ...
    abilityTo(abilityType) {
        ...
        if (!found) {
            throw new index_js_1.ConfigurationError(`${this.name} can ${Array.from(this.abilities.values()).map(ability => ability.constructor.name).join(', ')}. ` +
    ...
    attemptsTo(...activities) {
        return activities
            ...
            return previous
                .then(() => index_js_5.PerformActivities.as(this).perform(current));
     ...
     */
    answer(answerable) {
        return index_js_5.AnswerQuestions.as(this).answer(answerable);
    }
    /**
     * @inheritDoc
     */
    collect(artifact, name) {
        this.stage.announce(new index_js_2.ActivityRelatedArtifactGenerated(this.stage.currentSceneId(), this.stage.currentActivityId(), this.nameFrom(name || new index_js_4.Name(artifact.constructor.name)), artifact, this.stage.currentTime()));
    }
    /**
    ...
    acquireAbility(ability) {
        if (!(ability instanceof index_js_5.Ability)) {
            throw new index_js_1.ConfigurationError(`Custom abilities must extend Ability from '@serenity-js/core'. Received ${index_js_3.ValueInspector.typeOf(ability)}`);
        }
