...
const index_js_3 = require("../../questions/index.js");
const index_js_4 = require("../../questions/index.js");
const index_js_5 = require("../abilities/index.js");
const index_js_6 = require("../models/index.js");
/**
...
class Wait {
     ...
     */
    static minimumTimeout = index_js_6.Duration.ofMilliseconds(250);
    /**
     ...
     * @type {Duration}
     */
    static defaultPollingInterval = index_js_6.Duration.ofMilliseconds(500);
    /**
     ...
     */
    static minimumPollingInterval = index_js_6.Duration.ofMilliseconds(50);
    /**
 ...
 * @package
 */
class WaitFor extends Interaction_js_1.Interaction {
    duration;
    constructor(duration) {
        super((0, index_js_3.the) `#actor waits for ${duration}`);
        this.duration = duration;
    }
    async performAs(actor) {
        const duration = await actor.answer(this.duration);
        return index_js_5.ScheduleWork.as(actor).waitFor(duration);
    }
}
/**
 ...
 * @group Time
 */
class WaitUntil extends Interaction_js_1.Interaction {
    actual;
    expectation;
    pollingInterval;
    timeout;
    constructor(actual, expectation, pollingInterval, timeout) {
        super((0, index_js_3.the) `#actor waits until ${actual} does ${expectation}`);
        this.actual = actual;
     ...
     */
    async performAs(actor) {
        await index_js_5.ScheduleWork.as(actor).repeatUntil(() => actor.answer(this.expectation.isMetFor(this.actual)), {
            exitCondition: outcome => outcome instanceof index_js_4.ExpectationMet,
            delayBetweenInvocations: (invocation) => invocation === 0
                ? index_js_6.Duration.ofMilliseconds(0)
                : this.pollingInterval,
            timeout: this.timeout,
            errorHandler: (error, outcome) => {
                if (error instanceof index_js_1.ListItemNotFoundError) {
                    return; // ignore, lists might get populated later
                }
                if (error instanceof index_js_1.TimeoutExpiredError) {
                    throw index_js_1.RaiseErrors.as(actor).create(index_js_1.AssertionError, {
                        message: error.message + (0, index_js_2.d) ` while waiting for ${this.actual} to ${this.expectation}`,
