 ...
 * @group Expectations
 */
class Expectation extends index_js_4.Describable {
    functionName;
    predicate;
    /**
    ...
    static define(functionName, relationship, predicate) {
        return Object.defineProperty(function (...answerableArguments) {
            const description = typeof relationship === 'function' ? relationship(...answerableArguments)
                : (answerableArguments.length === 1 ? (0, index_js_3.the) `${{ toString: () => relationship }} ${answerableArguments[0]}`
                    : relationship);
            return new Expectation(functionName, description, async (actor, actualValue) => {
                const predicateArguments = await (0, index_js_1.asyncMap)(answerableArguments, answerableArgument => actor.answer(answerableArgument));
                ...
                const expectationDetails = index_js_2.ExpectationDetails.of(functionName, ...predicateArguments);
                ...
                return result
                    ? new index_js_2.ExpectationMet(descriptionText, expectationDetails, expected, actual)
                    : new index_js_2.ExpectationNotMet(descriptionText, expectationDetails, expected, actual);
            });
    ...
    static thatActualShould(relationshipName, expectedValue) {
        return ({
            soThat: (simplifiedPredicate) => {
                const message = relationshipName + ' ' + (0, index_js_1.d) `${expectedValue}`;
                return new Expectation('unknown', message, async (actor, actualValue) => {
                    ...
                    const expectationDetails = index_js_2.ExpectationDetails.of('unknown');
                    return result
                        ? new index_js_2.ExpectationMet(message, expectationDetails, expected, actual)
                        : new index_js_2.ExpectationNotMet(message, expectationDetails, expected, actual);
                });
            },
        });
    }
     ...
     */
    isMetFor(actual) {
        return Question_js_1.Question.about(this.getDescription(), actor => this.predicate(actor, actual));
    }
    /**
     * @inheritDoc
     */
