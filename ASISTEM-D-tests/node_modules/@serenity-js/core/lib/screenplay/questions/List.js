...
const index_js_3 = require("./expectations/index.js");
/**
 ...
 * @group Questions
 */
class List extends Question_js_1.Question {
    collection;
    subject;
    static of(collection) {
        if (Question_js_1.Question.isAMetaQuestion(collection)) {
            return new MetaList(collection);
        }
        return new ArrayList(collection);
    }
    constructor(collection) {
        super((0, index_js_2.d) `${collection}`);
    ...
    async answeredBy(actor) {
        ...
        if (!Array.isArray(collection)) {
            throw new index_js_1.LogicError((0, index_js_2.d) `A List has to wrap an Array-compatible object. ${collection} given.`);
        }
        return collection;
    }
    /**
...
class ArrayList extends List {
    ...
    count() {
        return Question_js_1.Question.about(`the number of ${this.toString()}`, async (actor) => {
            const items = await this.answeredBy(actor);
            return items.length;
        });
    }
    first() {
        return Question_js_1.Question.about(`the first of ${this.toString()}`, async (actor) => {
            const items = await this.answeredBy(actor);
            if (items.length === 0) {
                throw new index_js_1.ListItemNotFoundError((0, index_js_2.d) `Can't retrieve the first item from a list with 0 items: ${items}`);
            }
            return items[0];
        });
    }
    last() {
        return Question_js_1.Question.about(`the last of ${this.toString()}`, async (actor) => {
            const items = await this.answeredBy(actor);
            if (items.length === 0) {
                throw new index_js_1.ListItemNotFoundError((0, index_js_2.d) `Can't retrieve the last item from a list with 0 items: ${items}`);
            }
            return items.at(-1);
        });
    }
    nth(index) {
        return Question_js_1.Question.about(`the ${this.ordinal(index + 1)} of ${this.toString()}`, async (actor) => {
            const items = await this.answeredBy(actor);
            if (index < 0 || items.length <= index) {
                throw new index_js_1.ListItemNotFoundError(`Can't retrieve the ${this.ordinal(index)} item from a list with ${items.length} items: ` + (0, index_js_2.d) `${items}`);
            }
            return items[index];
        });
    }
...
class MetaList extends List {
    ...
    of(context) {
        return new MetaList(this.collection.of(context)).describedAs(this.toString() + (0, index_js_2.d) ` of ${context}`);
    }
    eachMappedTo(question) {
 ...
 * @package
 */
class Where extends Question_js_1.Question {
    collection;
    question;
    expectation;
    constructor(collection, question, expectation, originalSubject) {
        const prefix = collection instanceof Where
            ? ' and'
            : ' where';
        super(originalSubject + prefix + (0, index_js_2.d) ` ${question} does ${expectation}`);
        this.collection = collection;
    ...
    async answeredBy(actor) {
        try {
            ...
            for (const item of collection) {
                ...
                if (expectationOutcome instanceof index_js_3.ExpectationMet) {
                    results.push(item);
                }
            }
            return results;
            ...
            throw new index_js_1.LogicError((0, index_js_2.d) `Couldn't check if ${this.question} of an item of ${this.collection} does ${this.expectation}: ` + error.message, error);
        }
    }
}
/**
 ...
 * @package
 */
class EachMappedTo extends Question_js_1.Question {
    collection;
    mapping;
    constructor(collection, mapping, originalSubject) {
        super(originalSubject + (0, index_js_2.d) ` mapped to ${mapping}`);
        this.collection = collection;
 ...
 * @package
 */
class ForEachLoop extends Task_js_1.Task {
    collection;
    subject;
    fn;
