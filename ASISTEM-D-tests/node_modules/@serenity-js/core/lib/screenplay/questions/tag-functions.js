...
const Describable_js_1 = require("./Describable.js");
function the(...args) {
    if (index_js_1.ValueInspector.isPlainObject(args[0])) {
...
function createParameterToDescriptionMapper(options) {
    return (parameter) => parameter === undefined
        ? 'undefined'
        : Question_js_1.Question.formattedValue(options).of(parameter).toString();
}
function createParameterValueToDescriptionMapper(options) {
    return async (actor, parameter) => parameter instanceof Describable_js_1.Describable
        ? parameter.describedBy(actor)
        : actor.answer(Question_js_1.Question.formattedValue(options).of(parameter));
}
...
function templateToQuestion(templates, parameters, descriptionMapper, valueMapper) {
    ...
    return Question_js_1.Question.about(description, async (actor) => {
        const descriptions = await (0, index_js_1.asyncMap)(parameters, parameter => valueMapper(actor, parameter));
        return interpolate(templates, descriptions);
    }, (context) => templateToQuestion(templates, parameters.map(parameter => Question_js_1.Question.isAMetaQuestion(parameter)
