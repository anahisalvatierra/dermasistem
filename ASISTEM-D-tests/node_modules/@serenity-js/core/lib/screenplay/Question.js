...
const Describable_js_1 = require("./questions/Describable.js");
const tag_functions_js_1 = require("./questions/tag-functions.js");
const Unanswered_js_1 = require("./questions/Unanswered.js");
/**
 ...
 */
class Question extends Describable_js_1.Describable {
    ...
    static fromArray(source, options) {
        ...
        const description = source.length === 0
            ...
            : Question.about(formatter.format(source), async (actor) => {
                const descriptions = await (0, index_js_2.asyncMap)(source, item => item instanceof Describable_js_1.Describable
        ...
        return Question.about(description, async (actor) => {
            return await (0, index_js_2.asyncMap)(source, item => actor.answer(item));
        });
    }
    /**
    ...
    static staticFieldDescription(target, key) {
        ...
        const originalSubject = (0, index_js_2.f) `${target}`;
    ...
    static methodDescription(target, parameters) {
        ...
        const parameterDescriptions = [
            '(', parameters.map(p => (0, index_js_2.f) `${p}`).join(', '), ')',
        ].join('');
     ...
     */
    as(mapping) {
        return Question.about((0, index_js_2.f) `${this}.as(${mapping})`, async (actor) => {
 ...
 * @package
 */
class QuestionStatement extends Interaction_js_1.Interaction {
    body;
    answer = new Unanswered_js_1.Unanswered();
    ...
    [node_util_1.default.inspect.custom](depth, options, inspect) {
        return (0, index_js_2.inspectedObject)(this.answer)(depth, options, inspect);
    }
    describedAs(description) {
        super.setDescription(Question.isAMetaQuestion(description)
            ? description.of(this)
...
class MetaQuestionStatement extends QuestionStatement {
    ...
    of(answerable) {
        return Question.about((0, tag_functions_js_1.the) `${this} of ${answerable}`, actor => actor.answer(this.metaQuestionBody(answerable)));
    }
}
/**
 * @package
...
class IsPresent extends Question {
    ...
    constructor(question) {
        super((0, index_js_2.f) `${question}.isPresent()`);
...
async function recursivelyAnswer(actor, answerable, description, currentRecursion = 0) {
    if (currentRecursion >= maxRecursiveCallsLimit) {
        throw new index_js_1.LogicError(`Question.fromObject() has reached the limit of ${maxRecursiveCallsLimit} recursive calls while trying to resolve ${description}. Could it contain cyclic references?`);
    }
    const answer = await actor.answer(answerable);
    if ((0, isRecord_js_1.isRecord)(answer)) {
...
class QuestionAboutFormattedValue extends Question {
    ...
    async describedBy(actor) {
        const unanswered = !this.context
            || !this.context['answer']
            || Unanswered_js_1.Unanswered.isUnanswered(this.context.answer);
...
class ValueFormatter {
    ...
    format(value) {
        ...
        if (index_js_2.ValueInspector.isPromise(value)) {
            return 'Promise';
        }
        ...
        if (index_js_2.ValueInspector.isDate(value)) {
            return `Date(${value.toISOString()})`;
        }
        if (value instanceof RegExp) {
            return `${value}`;
        }
        if (index_js_2.ValueInspector.hasItsOwnToString(value)) {
            return `${this.trim(value.toString())}`;
        }
        if (index_js_2.ValueInspector.isPlainObject(value)) {
        ...
        if (typeof value === 'object') {
            const entries = (0, significantFields_js_1.significantFieldsOf)(value)
