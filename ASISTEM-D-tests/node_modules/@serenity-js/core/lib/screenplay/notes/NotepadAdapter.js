...
const index_js_2 = require("../questions/index.js");
const TakeNotes_js_1 = require("./TakeNotes.js");
/**
...
class NotepadAdapter {
     ...
     */
    has(subject) {
        return Question_js_1.Question.about(`a note of ${String(subject)} exists`, actor => {
            return TakeNotes_js_1.TakeNotes.as(actor).notepad.has(subject);
        });
    }
    /**
     ...
     */
    get(subject) {
        return Question_js_1.Question.about(`a note of ${String(subject)}`, actor => {
            return TakeNotes_js_1.TakeNotes.as(actor).notepad.get(subject);
        }).describedAs(Question_js_1.Question.formattedValue());
    }
    /**
     ...
     */
    delete(subject) {
        return Question_js_1.Question.about(`#actor deletes a note of ${String(subject)}`, actor => {
            return TakeNotes_js_1.TakeNotes.as(actor).notepad.delete(subject);
        });
    }
    /**
     ...
     */
    clear() {
        return Interaction_js_1.Interaction.where((0, index_js_2.the) `#actor clears ${new NumberOfNotes()} from their notepad`, actor => {
            return TakeNotes_js_1.TakeNotes.as(actor).notepad.clear();
        });
    }
    /**
     ...
     */
    size() {
        return Question_js_1.Question.about((0, index_js_2.the) `${new NumberOfNotes()}`, async (actor) => {
            return TakeNotes_js_1.TakeNotes.as(actor).notepad.size();
        });
    }
    /**
     ...
     */
    toJSON() {
        return Question_js_1.Question.about('notepad serialised to JSON', async (actor) => {
            return TakeNotes_js_1.TakeNotes.as(actor).notepad.toJSON();
        });
    }
    /**
     * @inheritDoc
 ...
 * @package
 */
class ChainableNoteSetter extends Interaction_js_1.Interaction {
    notes;
    constructor(notes) {
    ...
    async performAs(actor) {
        const notepad = TakeNotes_js_1.TakeNotes.as(actor).notepad;
...
class DescriptionOfNotes extends Question_js_1.Question {
    notes;
    options;
    ...
    async answeredBy(actor) {
        ...
        const list = await (0, index_js_1.asyncMap)(noteNames, async (noteName) => {
            const label = `${noteName}:`.padEnd(maxWidth + 1);
            const noteDescription = await actor.answer(Question_js_1.Question.formattedValue(this.options).of(this.notes[noteName]));
...
class NumberOfNotes extends Question_js_1.Question {
    constructor() {
        super('notes');
    }
    async answeredBy(actor) {
