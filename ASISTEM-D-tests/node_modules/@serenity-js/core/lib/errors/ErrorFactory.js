...
const equal_js_1 = require("tiny-types/lib/objects/equal.js");
const index_js_1 = require("../io/index.js");
const Unanswered_js_1 = require("../screenplay/questions/Unanswered.js");
const AnsiDiffFormatter_js_1 = require("./diff/AnsiDiffFormatter.js");
/**
...
class ErrorFactory {
    formatter;
    constructor(formatter = new AnsiDiffFormatter_js_1.AnsiDiffFormatter()) {
...
class DiffValue {
    ...
    constructor(name, value) {
        this.value = value;
        this.nameAndType = `${name} ${index_js_1.ValueInspector.typeOf(value)}`;
    ...
    type() {
        return index_js_1.ValueInspector.typeOf(this.value);
    }
    isComplex() {
        return (typeof this.value === 'object' || node_util_1.types.isProxy(this.value))
            && !(this.value instanceof RegExp)
            && !(this.value instanceof Unanswered_js_1.Unanswered);
    }
    isArray() {
        return Array.isArray(this.value);
    }
    isComparableAsJson() {
        if (!this.value || this.value instanceof Unanswered_js_1.Unanswered) {
            return false;
        }
        return index_js_1.ValueInspector.isPlainObject(this.value)
            || this.value['toJSON'];
    }
    toString() {
    ...
    summaryOf(value) {
        ...
        if (isDefined && (index_js_1.ValueInspector.isPrimitive(value) || value instanceof RegExp)) {
            return String(value);
        }
        return undefined;
    }
...
class Diff {
    ...
    renderActualValue(expected, actual) {
        const lines = (0, index_js_1.inspected)(actual.value)
            .split('\n')
    ...
    renderArrayDiff(expected, actual) {
        const changes = (0, diff_1.diffArrays)(expected.value, actual.value, { comparator: equal_js_1.equal });
        const lines = changes.reduce((acc, change) => {
            const items = change.value;
            return acc.concat(items.map(item => DiffLine.changed(change, (0, index_js_1.inspected)(item, { compact: true }))
                .prepend('  ')
                .prependMarker()));
        }, []);
        const { added, removed } = this.countOf(changes);
