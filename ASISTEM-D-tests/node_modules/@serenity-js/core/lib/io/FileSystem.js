...
class FileSystem {
    ...
    os;
    directoryMode;
    constructor(root, fs = graceful_fs_1.default, os = nodeOS, directoryMode = Number.parseInt('0777', 8) & (~process.umask())) {
        this.root = root;
        this.fs = fs;
        this.os = os;
        ...
        try {
            ...
            if (stat.isFile()) {
                ...
                for (const entry of entries) {
                    await this.remove(absolutePath.join(new Path_js_1.Path(entry)));
                }
    ...
    temporaryDirectory(prefix) {
        const osTemporaryDirectory = Path_js_1.Path.from(this.os.tmpdir());
        const subDirectory = Path_js_1.Path.from(prefix || '/');
        ...
        return Path_js_1.Path.from(customTemporaryDirectory);
    }
}
