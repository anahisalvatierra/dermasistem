...
const { sep } = upath_1.default;
const path = upath_1.default;
class Path extends tiny_types_1.TinyType {
    ...
    static fromSanitisedString(value) {
        const normalised = path.normalize(value).replaceAll(/["'/:\\]/gi, ''), extension = path.extname(normalised), basename = path.basename(normalised, extension), filename = (0, filenamify_1.default)(basename, { replacement: '-', maxLength: 250 })
            .trim()
            .replaceAll(/[\s-]+/g, '-');
