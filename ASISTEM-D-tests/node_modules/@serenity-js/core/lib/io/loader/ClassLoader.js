...
class ClassLoader {
    ...
    instantiate(description) {
        ...
        if (typeof requiredType !== 'function') {
            throw new index_js_1.ConfigurationError((0, format_js_1.d) `Module ${descriptor.moduleId} doesn't seem to export ${descriptor.className}. Exported members include: ${Object.keys(requiredModule).join(', ')}`);
        }
        ...
        if (needsParameter && requiredType.length > 1) {
            throw new index_js_1.ConfigurationError((0, format_js_1.d) `${descriptor.className} exported by ${descriptor.moduleId} must be a class with a static fromJSON(config) method or a function that accepts a single config parameter: ${descriptor.parameter}`);
        }
        if (!needsParameter && requiredType.length > 0) {
            throw new index_js_1.ConfigurationError((0, format_js_1.d) `${descriptor.className} exported by ${descriptor.moduleId} must be a parameterless function since no config parameter is specified`);
        }
        try {
            ...
            const errorMessage = [
                (0, format_js_1.d) `${descriptor.className} exported by ${descriptor.moduleId} must be either:`,
                descriptor.parameter && (0, format_js_1.d) `- a class with a static fromJSON(config) method`,
                ...
                descriptor.parameter && (0, format_js_1.d) `where config is: ${descriptor.parameter}`,
            ].filter(Boolean).join('\n');
            throw new index_js_1.ConfigurationError(errorMessage);
        }
    }
    ensureDefined(value, operation) {
        if (value === undefined || value === null) {
            throw new index_js_1.ConfigurationError(`Calling ${operation} produced ${value}, which might indicate a configuration or programming error`);
        }
        return value;
    }
}
