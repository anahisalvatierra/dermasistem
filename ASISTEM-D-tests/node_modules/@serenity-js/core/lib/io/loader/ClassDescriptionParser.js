...
class ClassDescriptionParser {
    static validClassNamePattern = /^[A-Za-z]\w+$/;
    ...
    parse(classDescription) {
        if (!this.looksLikeClassDescription(classDescription)) {
            throw new index_js_1.ConfigurationError((0, format_js_1.d) `${classDescription} is not a valid class description. Valid class description must be a string or an Array with 1-2 items, where the first item is a string.`);
        }
        ...
        if (rest.length > 0) {
            throw new index_js_1.ConfigurationError(`${classDescription} is not a valid class description. Valid class description must be:\n` +
        ...
        if (desiredClassName === '') {
            throw new index_js_1.ConfigurationError(`Invalid class name in "${moduleIdAndClassDescription}". If you want to import the default export from a given module, please use the module ID.\n` +
        ...
        if (!ClassDescriptionParser.validClassNamePattern.test(className)) {
            throw new index_js_1.ConfigurationError(`"${className}" doesn't seem like a valid JavaScript class name in "${moduleIdAndClassDescription}"`);
        }
        if (!moduleId.startsWith('./')) {
            const result = (0, validate_npm_package_name_1.default)(moduleId);
            if (!result.validForNewPackages) {
                throw new index_js_1.ConfigurationError(`"${moduleId}" doesn't seem like a valid module id:\n${result.errors.map(error => `- ${error}\n`)}`);
            }
        }
        return {
            moduleId,
