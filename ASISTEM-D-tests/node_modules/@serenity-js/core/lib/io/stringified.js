...
const Question_js_1 = require("../screenplay/Question.js");
const ValueInspector_js_1 = require("./reflection/ValueInspector.js");
...
function stringified(value, config) {
    ...
    if (ValueInspector_js_1.ValueInspector.isPromise(value)) {
        return markAs('Promise', markQuestions);
    }
    if (Question_js_1.Question.isAQuestion(value)) {
        return markAs(value.toString(), markQuestions);
    }
    if (ValueInspector_js_1.ValueInspector.isDate(value)) {
        return value.toISOString();
    }
    if (ValueInspector_js_1.ValueInspector.hasItsOwnToString(value)) {
        return value.toString();
    }
    if (ValueInspector_js_1.ValueInspector.isInspectable(value)) {
        return value.inspect();
    }
    if (ValueInspector_js_1.ValueInspector.isFunction(value)) {
        return hasName(value)
            ? value.name
            : markAs(`Function`, true);
    }
    if (!ValueInspector_js_1.ValueInspector.hasCustomInspectionFunction(value) && ValueInspector_js_1.ValueInspector.isPlainObject(value) && isSerialisableAsJSON(value)) {
