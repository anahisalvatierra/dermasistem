"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.SceneFinished = void 0;
const tiny_types_1 = require("tiny-types");
const index_js_1 = require("../model/index.js");
const index_js_2 = require("../screenplay/index.js");
const DomainEvent_js_1 = require("./DomainEvent.js");
/**
 * @group Events
 */
class SceneFinished extends DomainEvent_js_1.DomainEvent {
    sceneId;
    details;
    outcome;
    static fromJSON(o) {
        return new SceneFinished(index_js_1.CorrelationId.fromJSON(o.sceneId), index_js_1.ScenarioDetails.fromJSON(o.details), index_js_1.Outcome.fromJSON(o.outcome), index_js_2.Timestamp.fromJSON(o.timestamp));
    }
    constructor(sceneId, details, outcome, timestamp) {
        super(timestamp);
        this.sceneId = sceneId;
        this.details = details;
        this.outcome = outcome;
        (0, tiny_types_1.ensure)('sceneId', sceneId, (0, tiny_types_1.isDefined)());
        (0, tiny_types_1.ensure)('details', details, (0, tiny_types_1.isDefined)());
        (0, tiny_types_1.ensure)('outcome', outcome, (0, tiny_types_1.isDefined)());
    }
}
exports.SceneFinished = SceneFinished;
//# sourceMappingURL=SceneFinished.js.map